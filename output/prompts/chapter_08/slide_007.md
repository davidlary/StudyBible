...
- Noise pollution from anthropogenic sources is another environmental impact that can disrupt bird song. Anthropogenic noise, such as traffic and industrial noise, can mask bird vocalizations, making it difficult for birds to communicate with each other. For example, a study by [halfwerk2011negative] showed that traffic noise affected the vocalizations of the great tit (Parus major) and resulted in a reduction in song frequency and song complexity.
- } Global change is also affecting bird song, as the changes in temperature and precipitation patterns alter the timing of breeding and migration, which in turn affects the timing of vocalizations. For example, a study by [beecher2019climate] showed that increasing temperatures and changing precipitation patterns led to changes in the vocalizations of ovenbirds (Seiurus aurocapilla), a neotropical migratory songbird.
